    ...
    success: function (data) {
      ...
      if (data) {
        ...
        for (i in lottery.notSelArr) {
          $(`<div><img src="${lottery.notSelArr[i].headSrc}" alt=""></div>`).appendTo(".reload-img");
...
function timeOut() {
  ...
  for (let i = 0; i < peopleLi.length; i++) {
    $(peopleLi[i]).find("img").attr("src", lottery.notSelArr[lottery.imgPosition]?.headSrc);
    $(peopleLi[i]).find("span").text(lottery.notSelArr[lottery.imgPosition]?.name);
    lottery.imgPosition++;
    lottery.imgPosition = lottery.imgPosition > lottery.notSelArr.length - 1 ? 0 : lottery.imgPosition;
  }
...
let isLotteryRunning = false; // 用于跟踪抽奖是否正在进行中
...
$(".lottery-btn").click(function () {
  if (lottery.seletedArr.length >= 39) {
    ...
    // 节流优化性能，如果正在抽奖就直接返回出去。  不是的话就设置为true
    // 效果就是禁用用户在抽奖过程还一直点抽奖，防止多个定时器一直叠加造成页面数据不对。
    if (isLotteryRunning) {
      return;
      // return alert("抽奖过程请勿点击!");
    }
    ...
    countdownElement = $(this).siblings(".lottery-absolute-timerout"); // 获取当前点击按钮同级的展示倒计时的盒子
    ...
    /* 获取当前点击开始按钮身上的id(每种类型的开始按钮都绑定了不同的id名称)
       ...
       例如:是特等奖的按钮,如果总共抽奖的次数小于1,说明还没抽奖
              同时判断按钮的状态是否 未点击状态(lottery['f'] == true)
                是的话当前抽奖类型的抽奖次数加1
    ...
    switch ($(this).attr("id")) {
      case "oneBtn":
        if (lottery_one_num != 1) {
          // 抽奖倒计时，传入 参与抽奖人员列表、当前点击按钮同级的展示倒计时的DOM、当前奖项的按钮DOM、各奖项的奖品DOM、抽奖倒计时长
        ...
        // 倒计时结束就将记录是否正在抽奖的变量初始化  节流优化
        isLotteryRunning = false;
        break;
      case "twoBtn":
        if (lottery_two_num != 3) {
          lotteryTimeout(peopleLi, countdownElement, "twoBtn", lottery_prize_Element, 5);
          break;
        }
        clearTimeout(lottery["timer"]);
        $(".lottery-btn p").text("开始");
      ...
      case "threeBtn":
        if (lottery_three_num != 5) {
          lotteryTimeout(peopleLi, countdownElement, "threeBtn", lottery_prize_Element, 5);
      ...
      case "fourBtn":
        if (lottery_four_num != 10) {
          lotteryTimeout(peopleLi, countdownElement, "fourBtn", lottery_prize_Element, 5);
          break;
        }
        clearTimeout(lottery["timer"]);
        $(".lottery-btn p").text("开始");
      ...
      case "fiveBtn":
        if (lottery_five_num != 20) {
          lotteryTimeout(peopleLi, countdownElement, "fiveBtn", lottery_prize_Element, 5);
          break;
        }
        clearTimeout(lottery["timer"]);
        $(".lottery-btn p").text("开始");
...
/*
  params 
      peopleLi：抽奖名单
      lottery_btn_type： 抽奖按钮的奖项类型标识，用于判断当前奖项总数是否已抽完，没抽完该奖项的次数自增对应的数量
*/
function choujiang(peopleLi, lottery_btn_type) {
  console.log(countdown, "choujiang");
  // console.log(1, "来了");

  if ((lottery["f"] = !lottery["f"]) && countdown > 0) {
    // 抽奖倒计时过程中
    lottery_prize_Element.hide(); // 开启抽奖就隐藏当前奖项的奖品文字
    ...
    */
      ...
      let selectedObjects = randomlySelectObjects(lottery.notSelArr, lotteryType, everyDrawNum, totalNum);
    ...
    lottery_prize_Element.show(); // 开启抽奖就隐藏当前奖项的奖品文字
    countdownElement.hide(); // 显示倒计时文字

    // 如果lottery.f等于false,也就是抽奖结束，那么就将对应奖项类型的奖项总数自增，
    // 此属性用于如果当前奖项已经达到抽奖次数最大，那么就禁用，不给抽奖
    // 具体代码请看 $(".lottery-btn").click 中的switch内部的代码
    if (lottery_btn_type == "oneBtn") {
      lottery_one_num++;
    }
    if (lottery_btn_type == "twoBtn") {
      lottery_two_num += 3;
    }
    if (lottery_btn_type == "threeBtn") {
      lottery_three_num += 5;
    }
    if (lottery_btn_type == "fourBtn") {
      lottery_four_num += 5;
    }
    if (lottery_btn_type == "fiveBtn") {
      lottery_five_num += 5;
    }
    ...
    if (lottery.rewardInfo.length) {
      ...
      for (i in lottery.rewardInfo) {
        ...
        for (let j = 0; j < peopleLi.length; j++) {
          ...
          $(peopleLi[j])
            .find("img")
            .attr("src", lottery.rewardInfo[j] ? lottery.rewardInfo[j].rewardSrc : "images/question.png");
          $(peopleLi[j])
            .find("span")
            .text(lottery.rewardInfo[j] ? lottery.rewardInfo[j].rewardName : "");
        }
      }
    } else {
      //没数据的话则直接显示问号头像
...
function randomlySelectObjects(arr, lotteryType, everyDrawNum, totalNum) {
  ...
  while (selected.length < everyDrawNum && arr.length > 0) {
    ...
    if (!selected.includes(arr[randomIndex])) {
      // 中奖等级标识 1=特等奖，2=一等奖，3=二等奖，4=三等奖，5=幸运奖
  ...
  if (zhongjiangArr.length) {
    ...
    zhongjiangArr.forEach(function (item, i) {
      if (item[0].tag >= 1 && item[0].tag <= 5) {
        ...
        var outer = $("<div>").addClass("lottery-level-outer").attr("data-key", item[0].tag);
...
let countdown = 0; // 抽奖秒数
// 抽奖倒计时 start
/*
  params
    userList：参与抽奖的用户列表
    ele: 当前奖项展示的计数器DOM
    btnType: 当前点击的是哪个奖项中的开始按钮
    lottery_prize_ele：当前奖项的奖品DOM
    countTime：各奖项倒计时的时间
*/

function lotteryTimeout(userList, countdownEle, btnType, lottery_prize_ele, countTime) {
  let countdownElement = countdownEle;
  countdownElement.text(countTime);
  let lottery_prize_Element = lottery_prize_ele;
  let countdownInterval = null; // 定时器
  countdown = countTime;
  // 如果计时器还没走完，那就抽奖。 等于0就再次触发抽奖按钮，让lottery.f等于true，这样就会走抽奖函数内部的else抽奖停止逻辑

  if (countdown > 0) {
    choujiang(userList);
    countdownElement.show(); // 显示倒计时文字
  }
  ...
  countdownInterval = setInterval(function () {
    ...
    // countdownElement.show(); // 显示倒计时文字
    countdownElement.text(countdown); // 将倒计时的数字显示到 当前奖项的显示倒计时DOM中，呈现到页面上
    if (countdown <= 0) {
      clearInterval(countdownInterval); // 清除抽奖定时器
      // 抽奖倒计时完毕
      countdownElement.hide(); // 隐藏倒计时文字
      lottery_prize_Element.show(); // 显示当前奖项的奖品文字
      countdown = countTime;
      // 再次触发一次抽奖，此时lottery.f的值为false，所以不会继续抽奖，而是公布中奖名单。
      // 也就是走choujiang函数里面else的代码逻辑
      choujiang(userList, btnType);
      // 倒计时结束就将记录是否正在抽奖的变量初始化  节流优化
      isLotteryRunning = false;
    }
  }, 1200); // 设置新的定时器   让实际每秒延迟200毫秒，否则页面展示会出现数字突然自动加快的BUG
  // // 定时器 end
}
